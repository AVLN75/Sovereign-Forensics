cat << 'SOVEREIGN_FINAL' > sovereign_master_build.sh
...
def index_to_cloud():
    ...
    for root, _, files in os.walk(repo_path):
        ...
        for f in files:
            if f.endswith(('.py', '.sh', '.conf', '.json')):
                items.append(os.path.join(root, f))
    
    manifest = json.dumps({
        "ts": str(datetime.datetime.now()),
        "status": "DIRECT_RCLONE_ACTIVE",
        "layer": "DUAL_PUBLIC_PRIVATE",
        "files": items
    }, indent=4)
    
    # Stream manifest directly to GDrive
    ...
    print("[CLOUD] System Index Synced.")
...
async def handle_stream(websocket, path):
    ...
    async for message in websocket:
        ...
        # Pipe directly to GDrive via rclone rcat (spawned per batch or session)
        print(f"Captured: {data.get('type')} | Content: {str(data.get('content'))[:40]}")
        # Manual log update logic here...
...
chmod +x sovereign_master_build.sh
./sovereign_master_build.sh